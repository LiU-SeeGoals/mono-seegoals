...
static KICKER_Settings settings = {.max_charges_per_kick = 6, .safe_discharge_wait_us=25, .charge_wait_us = 25000, .discharge_wait_us = 130, .charges_since_last_kick = 0};
...
void KICKER_KickSafe()
{
    ...
    __disable_irq();
    HAL_GPIO_WritePin(KICKER_DISCHARGE2_GPIO_Port, KICKER_DISCHARGE2_Pin, GPIO_PIN_RESET);
    wait(settings.safe_discharge_wait_us); // TODO replace with hardware timer
    HAL_GPIO_WritePin(KICKER_DISCHARGE2_GPIO_Port, KICKER_DISCHARGE2_Pin, GPIO_PIN_SET);
    __enable_irq();
    settings.charges_since_last_kick = 0;
    // LOG_DEBUG("Kicking!\r\n");
