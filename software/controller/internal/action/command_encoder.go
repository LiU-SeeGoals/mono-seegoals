...
// Protocol: 20 bytes total (12 bytes under NRF24 32-byte limit)
//
// Byte Layout:
...
// [12-13] : Direction X (int16) - normalized [-100, 100]
// [14-15] : Direction Y (int16) - normalized [-100, 100]
...
// EncodeCommand converts a robot_action.Command to a 30-byte binary buffer
func EncodeCommand(cmd *robot_action.Command) ([]byte, error) {
	...
	// [12-13] Direction X (int16, normalized -100 to 100)
	dirX := normalizeDirection(cmd.Direction.X)
	binary.LittleEndian.PutInt16(buf[offsetDirectionX:], dirX)

	// [14-15] Direction Y (int16, normalized -100 to 100)
	dirY := normalizeDirection(cmd.Direction.Y)
...
// DecodeCommand converts a 30-byte binary buffer back to a robot_action.Command
...
// normalizeDirection converts int32 direction to int16 normalized range [-100, 100]
// Clamps the value to prevent overflow
func normalizeDirection(val int32) int16 {
	if val < -100 {
		return -100
	}
	if val > 100 {
